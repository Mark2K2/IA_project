...
def retrieval_by_color(llistaim, color_labels, preg):
    ...
    images = [ llistaim[x]
        ...
        and not list.append(x)
    ]

    return np.array(images), list
...
def retrieval_by_shape(llistaim, class_labels, preg):
    ...
    images = [ llistaim[x]
        ...
        if any(shape.title() == class_label for shape in preg) and not list.append(x)
    ]
...
def parse_search_pregunta(pregunta, colors, classes):
    ...
    if type != 'exit':
        ...
        pregunta = {
            'shape': shapep,'color': colorp
        }

    return pregunta, type
